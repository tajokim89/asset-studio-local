...
console.log(JSON.stringify({desc,rects,beats4:deriveWalkBeatLabels('walk4',4),beats6:deriveWalkBeatLabels('walk',6),repeated,moving,badGrid:capture(()=>deriveSpriteFrameRectangles(desc,{width:127,height:32})),budget:capture(()=>deriveSpriteFrameRectangles(desc,{width:32768,height:32768})),gateFail:resultWalkQaGate(desc,{status:'FAIL'},true),gatePending:resultWalkQaGate(desc,{status:'PASS'},false),gatePass:resultWalkQaGate(desc,{status:'PASS'},true)}));
'''
...
def test_pure_descriptor_rectangles_and_fail_closed_budgets():
    ...
    assert out['badGrid']['ok'] is False and out['budget']['ok'] is False
...
def test_deterministic_repeat_failure_and_manual_gate():
    out=runtime()
    assert out['repeated']['status']=='FAIL' and out['moving']['status']=='PASS'
    assert out['gateFail']['allowed'] is False and out['gatePending']['allowed'] is False and out['gatePass']['allowed'] is True

def test_result_card_has_inline_controls_cleanup_and_manual_qa_state():
    source=JS_PATH.read_text(); render=decl(source,'renderAssetResultTray')+decl(source,'mountResultSpritePlayer')
    for token in ('result-sprite-animation','play-pause','previous-frame','next-frame','animation-fps','발 교대 확인 필요','manual-walk-pass'):
        assert token in render
    for token in ('visibilitychange','clearInterval','imageSmoothingEnabled'):
        assert token in source
    adopt=decl(source,'adoptResult')
    assert 'resultWalkQaGate' in adopt
    update=decl(source,'updateResultWalkQaUi')
    assert "card.querySelector('[data-result-action=\"manual-walk-pass\"]')" in update
