...
REF_SRC=Path('/Users/tajokim/.hermes/image_cache/asset-studio-local/whitelist_walk4/walk4_whitelist_1783564486.png')

img=Image.open(REF_SRC).convert('RGBA')
w,h=img.size
cell=w//4
ref=img.crop((cell,0,cell*2,h))
buf=io.BytesIO()
...
Generate a STRICT 4-frame S/front-facing in-place walk sprite sheet.

Critical registration requirement: every frame must be onion-skin registered to the same root. The hood peak, goggles midpoint, torso center, pelvis/root, belt center, and foot baseline must stay on the same pixel coordinates inside each cell. Do not let the character body drift left or right. Do not center by silhouette; keep the anatomical root fixed.

Frame beats left to right:
1 left foot contact forward/down, right foot back, broom hand counter-swing
2 passing pose with both feet near body center, same root
3 right foot contact forward/down, left foot back, opposite of frame 1
4 passing pose connecting cleanly back to frame 1

PASS only if it reads as a human biped walk-in-place with alternating feet while the body root stays fixed. If the action reads as idle, tapping, skating, moonwalk, hop, dance, or body slide, it is FAIL.
...
One horizontal row, exactly 4 equal cells, wide #00FF00 gutters, flat chroma green background, no text, no numbers, no watermark, no VFX, no particles, no background scene.'''
...
if not res.get('success') or not res.get('path'):
    out['success']=False
    (OUT/'walk4_refroot_result.json').write_text(json.dumps(out, ensure_ascii=False, indent=2))
    raise SystemExit(json.dumps(out, ensure_ascii=False, indent=2))

stem=f'walk4_refroot_{int(time.time())}'
dst=OUT/f'{stem}.png'
